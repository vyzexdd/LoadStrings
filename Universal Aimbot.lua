...
local Window = Rayfield:CreateWindow({
    Name = "▶ Uniersal Aimbot ◀",
    LoadingTitle = "Loading...",
    LoadingSubtitle = "by xzieace",
    ConfigurationSaving = {
        Enabled = true,
...
local function getESPColor(targetPlayer)
    if targetPlayer.Team == plr.Team then
...
local function createESP(player)
    ...
    player.CharacterAdded:Connect(function(char)
        ...
        if espNamesEnabled then
            local billboard = char:FindFirstChild("ESPName")
...
local function removeESP(player)
    if player.Character then
        ...
        local billboard = player.Character:FindFirstChild("ESPName")
        if billboard then billboard:Destroy() end
    end
end
